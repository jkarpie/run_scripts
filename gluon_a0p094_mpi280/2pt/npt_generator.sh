...
nucOps=${list_dir}/generator_nuc_${LG}.list
...
# I/Os
########################
#### ORIGINAL CFG FILES FULL NAME IS LIKE cl21_32_64_b6p3_m0p2390_m0p2050_cfg_1000.lime
#CFGPREF_CHR="/work2/06377/tg856768/frontera/isoClover/${BETA}/${ENSEM}/cfgs/${ENSEM}_cfg"
#### MY CFG FILES FULL NAME IS LIKE cl21_32_64_b6p3_m0p2390_m0p2050-10700_cfg_11000.lime
CFGPREF_CHR="${PROJ}/cfgs/${ENSEM}-${stream}_cfg"

#### ORIGINAL EIG FILES FULL NAME IS LIKE cl21_32_64_b6p3_m0p2390_m0p2050.3d.eigs.mod4350
#CVEC=${PROJ}/eigs_mod/${ENSEM}.3d.eigs
#### MY EIG FILES FULL NAME IS LIKE cl21_32_64_b6p3_m0p2390_m0p2050-10700_eigen_z0_light.11000.eig
...
LOG=''; OUT=''
if [ $PHASE == '0.00' ]; then
    #### ORIGINAL PERAM FILES FULL NAME IS LIKE cl21_32_64_b6p3_m0p2390_m0p2050.prop.n192.light.t0_0.sdb1020
    #PERAM=/work2/06377/tg856768/frontera/isoClover/${BETA}/${ENSEM}/prop_db/${ENSEM}.prop.n${NVEC}.light.t0_${TOFFSET}
    #### MY PERAM FILES FULL NAME IS LIKE cl21_32_64_b6p3_m0p2390_m0p2050-10700_z2_light_peram.11000.T60.peram
...
## HERE IS WHERE I REPLACE COLIN's PYTHON SETUP WITH THE REDSTAR INTERNAL GENERATORS

cat << EOF > ${DUMP}/run$CFG/generate_redstar_2pt.ini.xml
<?xml version="1.0"?>
<GenerateXML>
  <runmode>default</runmode>
  <include_all_rows>false</include_all_rows>
  <default_row>1</default_row>
  <moms>
      <elem>$MOMX $MOMY $MOMZ</elem>
  </moms>
  <!-- <flavor><twoI>2</twoI> <threeY>0</threeY> <twoI_z>2</twoI_z></flavor> -->
  <t_sources>0</t_sources>
  <sink_ops_list>${nucOps}</sink_ops_list>
  <source_ops_list>${nucOps}</source_ops_list>
  <ops_xmls>
    <elem>${PYDIR}/nuc_op_lists/all_nuc.ops.xml</elem>
  </ops_xmls>
</GenerateXML>
EOF


generate_redstar_2pt ${DUMP}/run$CFG/generate_redstar_2pt.ini.xml ${DUMP}/run$CFG/npt.xml


cat <<EOF > nucleon_control.xml$CFG
<?xml version="1.0"?>
<RedstarNPt>
<Param>
  <version>12</version>
  ...
  <Nt_corr>${TCORR}</Nt_corr>
  <bc_spec>-1</bc_spec>

  <convertUDtoL>false</convertUDtoL>
  <convertUDtoS>false</convertUDtoS>

  <ensemble>fred</ensemble>

  <Layout>
    <lattSize>${LSIZE} ${LSIZE} ${LSIZE} ${TSIZE}</lattSize>
    <decayDir>3</decayDir>
  </Layout>
EOF

tail --lines=+2  ${DUMP}/run$CFG/npt.xml >> nucleon_control.xml$CFG

cat << EOF >> nucleon_control.xml$CFG
</Param>
<DBFiles>
  <proj_op_xmls></proj_op_xmls>
  <corr_graph_bin>corr_graph.bin</corr_graph_bin>
</DBFiles>
