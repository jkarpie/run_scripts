...
source /users/karpiejo/scratch/chromaform1/env.sh
source /users/karpiejo/scratch/chromaform1/env_extra.sh
source /users/karpiejo/scratch/chromaform1/env_extra_chroma.sh


#INPUTS
#TOFFSET MOMX MOMY MOMZ CFG PHASEZ 
...
export PROJ=/users/karpiejo/scratch/${BETA}/${ENSEM}/
#Location of chroma_python
PYDIR=/users/karpiejo/run_scripts/chroma_python
TSIZE=64
NVEC=64
#-------------------------------------------------------------------
...
run_dir="/users/karpiejo/scratch/redstar_run/"
LOG=''; OUT=''
if [ $PHASE == '0.00' ]; then
    ...
    BOP=/tmp/karpiejo/scratch/redstar_run/nuc_runs/${ENSEM}/2pt/unphased/t0_${TOFFSET}/momXYZ.${MOMX}.${MOMY}.${MOMZ}/run${CFG}/${ENSEM}.n${NVEC}.t0_${TOFFSET}.NtFwd_${NT_FWD}.baryon.colorvec
...
#CVEC=${DUMP}/run${CFG}/${ENSEM}_eigen_z0_light
#if [ $PHASE == '0.00' ]; then
#    PERAM=${DUMP}/run${CFG}/${ENSEM}_z0_light_peram
#else
#    PERAM=${DUMP}/run${CFG}/${ENSEM}_z${PHASE}_light_peram
#fi
#--------------------------------------------------------------------------------------------------
...
source /users/karpiejo/run_scripts/2pts/env.sh


export MPICH_GPU_SUPPORT_ENABLED=1

srun --cpu-bind=threads --threads-per-core=1 -c6 \
  $CHROMA $CHROMA_EX $GEOM -i bop_xmls/$XMLI >& ${LOG}/run${CFG}/BOP${CFG}
...
/users/karpiejo/run_scripts/2pts/run_redstar_int.2pt.no-nodes.sh \
