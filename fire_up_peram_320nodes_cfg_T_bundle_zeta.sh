...
file_stem="${cfg_stem}-${stream}_peram"
...
#PBS -l select=320:ncpus=64:ngpus=4:system=polaris
#PBS -q prod
#PBS -j oe
#PBS -k doe
#PBS -m n
...
for zeta in -2 -1 0 1 2
do

split --lines=\${NNODES_PER_JOB} --numeric-suffixes=1 --suffix-length=4 \$PBS_NODEFILE hostfiles/local_hostfile_z\${zeta}.${cfg}.\$PBS_JOBID.
c=${cfg}
T=0
for lh in hostfiles/local_hostfile_z\${zeta}.${cfg}.\$PBS_JOBID*
do
  echo "Cfg: \${c} T: \${T} Launching mpiexec w/ \${lh} "
cfg_file="/lus/grand/projects/SpectStructHadron/${cfg_stem}/${cfg_stem}-${stream}/cfgs/${cfg_stem}-${stream}_cfg_\${c}.lime"

file_s=${file_stem}_z\${zeta}_light.\${c}.T\${T}

mkdir -p  /lus/grand/projects/SpectStructHadron/${cfg_stem}/${cfg_stem}-${stream}/peram/\${c}
eig_file="/lus/grand/projects/SpectStructHadron/${cfg_stem}/${cfg_stem}-${stream}/eig/${cfg_stem}-${stream}_eigen_z0_light.\${c}.eig"
./make_peram_xml_zeta.sh xml/\${file_s}.ini.xml \$cfg_file \${eig_file} /lus/grand/projects/SpectStructHadron/${cfg_stem}/${cfg_stem}-${stream}/peram/\${c}/\${file_s}.peram \$T \$zeta

if [ -f /lus/grand/projects/SpectStructHadron/${cfg_stem}/${cfg_stem}-${stream}/peram/\${c}/\${file_s}.peram ]
then
rm /lus/grand/projects/SpectStructHadron/${cfg_stem}/${cfg_stem}-${stream}/peram/\${c}/\${file_s}.peram
fi
