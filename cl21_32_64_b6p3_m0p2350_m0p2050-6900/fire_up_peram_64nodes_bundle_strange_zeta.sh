...
file_stem="${cfg_stem}-${stream}_peram_strange"
...
split --lines=\${NNODES_PER_JOB} --numeric-suffixes=1 --suffix-length=2 \$PBS_NODEFILE hostfiles/local_hostfile_z\${zeta}.${cfg}.\$PBS_JOBID.
T=0
...
file_s=${file_stem}_z\${zeta}_strange.${cfg}.T\${T}

mkdir -p  /lus/grand/projects/SpectStructHadron/${cfg_stem}/${cfg_stem}-${stream}/peram/${cfg}
eig_file="/lus/grand/projects/SpectStructHadron/${cfg_stem}/${cfg_stem}-${stream}/eig/${cfg_stem}-${stream}_eigen_z0_light.${cfg}.eig"
./make_peram_strange_xml_zeta.sh xml/\${file_s}.ini.xml \$cfg_file \${eig_file} /lus/grand/projects/SpectStructHadron/${cfg_stem}/${cfg_stem}-${stream}/peram/${cfg}/\${file_s}.peram \$T \$zeta
